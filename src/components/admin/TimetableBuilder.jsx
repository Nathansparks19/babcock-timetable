...
Be concise and practical. Format as a numbered list.`
  ...
  const autoSchedule = async () => {
    ...
    const bookedSlotIds = entries
      .filter(e => e.lecturer_id === form.lecturer_id)
      .map(e => e.time_slot_id)

    const bookedRoomSlots = entries.map(e => `${e.room_id}_${e.time_slot_id}`)
    const availableRooms = rooms.filter(r => r.capacity >= (course?.enrollment_count || 0))

    const freeSlots = timeSlots.filter(s =>
      !unavailSlotIds.includes(s.id) && !bookedSlotIds.includes(s.id)
    )
    ...
    <div>
      ...
      {showForm && (
        <div style={styles.form}>
          ...
          {conflict?.isError && (
            <p style={styles.error}>{conflict.conflicts[0]}</p>
          )}
...
const styles = {
  ...
  select: {
    padding: '10px 14px', background: '#1e3a4a',
  ...
  autoScheduleBar: {
    ...
    borderRadius: '10px', padding: '16px', marginBottom: '16px', gap: '16px',
  },
  ...
  aiBox: {
    marginTop: '16px', borderTop: '1px solid rgba(255,150,0,0.2)', paddingTop: '16px',
  },
  aiTitle: { color: '#ffd200', fontWeight: 'bold', margin: '0 0 8px 0', fontSize: '14px' },
  ...
  slotEmpty: {
    ...
    borderRadius: '10px', padding: '16px', minHeight: '100px',
  },
  slotFilled: {
  ...
  slotCourse: { color: '#ffd200', fontWeight: 'bold', fontSize: '14px', margin: '0 0 2px 0' },
  ...
  removeBtn: {
    marginTop: '8px', background: 'rgba(255,100,100,0.1)', border: '1px solid rgba(255,100,100,0.2)',
    color: '#ff6b6b', padding: '4px 10px', borderRadius: '6px', cursor: 'pointer', fontSize: '11px',
  },
}